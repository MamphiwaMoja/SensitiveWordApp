...
public class SensitiveWordService {
    ...
    private final SensitiveWordDefinitionValidator definitionValidator;
    ...
    public SensitiveWordService(
            ...
            SensitiveWordDefinitionValidator definitionValidator
    ) {
    ...
    public SensitiveWordResponse create(CreateSensitiveWordRequest request) {
        ...
        MatchType matchType = request.matchType() == null ? MatchType.CONTAINS : request.matchType();
        boolean active = request.active() == null || request.active();

        definitionValidator.validate(request.word().trim(), matchType);

        if (active && sensitiveWordRepository.existsActiveRule(normalizedWord, matchType)) {
            throw new DuplicateSensitiveWordException("An active sensitive-word rule already exists for this word and match type");
        }

        SensitiveWord word = new SensitiveWord();
        word.setCategory(resolveCategory(request.categoryId()));
        word.setWord(request.word().trim());
        word.setReplacementValue(defaultIfBlank(request.replacementValue(), Constants.DEFAULT_REPLACEMENT));
        word.setMatchType(matchType);
        word.setSeverityLevel(request.severityLevel() == null ? 1 : request.severityLevel());
        word.setCaseSensitive(request.caseSensitive() != null && request.caseSensitive());
        word.setActive(active);
        if (!active) {
            word.setEffectiveTo(LocalDateTime.now(ZoneOffset.UTC));
        }
        word.setNotes(trimToNull(request.notes()));
    ...
    public SensitiveWordResponse update(Long id, UpdateSensitiveWordRequest request) {
        ...
        MatchType proposedMatchType = request.matchType() == null ? existing.getMatchType() : request.matchType();
        ...
        definitionValidator.validate(proposedWord, proposedMatchType);

        if (activeAfterUpdate
                && sensitiveWordRepository.existsActiveRuleExcludingId(normalizedWord, proposedMatchType, id)) {
            throw new DuplicateSensitiveWordException("Another active sensitive-word rule already exists for this word and match type");
        }
        ...
        if (request.replacementValue() != null) {
            existing.setReplacementValue(defaultIfBlank(request.replacementValue(), Constants.DEFAULT_REPLACEMENT));
        }
        if (request.matchType() != null) {
            existing.setMatchType(request.matchType());
        }
        if (request.severityLevel() != null) {
            existing.setSeverityLevel(request.severityLevel());
        }
        if (request.caseSensitive() != null) {
            existing.setCaseSensitive(request.caseSensitive());
        }
        if (request.active() != null) {
            existing.setActive(request.active());
            if (!request.active()) {
                existing.setEffectiveTo(LocalDateTime.now(ZoneOffset.UTC));
            } else {
